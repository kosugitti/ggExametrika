...
### 依存パッケージ（DESCRIPTION準拠、v0.0.35時点）
...
補足: - gridExtra:
v0.0.35でDependsからImportsに移動（CRANではDependsの最小化を推奨） -
ggrepel: plotCRV_gg/plotRRV_ggのshow_labelsで使用（v0.0.24追加） -
tidyr: plotICBR_gg/plotICRP_ggのpivot_longerで使用（v0.0.18追加） -
exametrikaはSuggestsに入れる（CRAN対応のため） - visNetwork:
v1.1.0以降で導入予定（インタラクティブDAG編集用）

### CRANチェックリスト（v0.0.35時点）

R CMD check –as-cran で 0 errors \| 0 warnings \| 0
notes（v0.0.35で達成）

NEWS.mdの更新（v0.0.35まで全バージョン記録済み）

DESCRIPTIONのバージョン更新（v0.0.35）

Title Case化・Description CRAN準拠化

LICENSE ファイル作成（MIT + file LICENSE）

cran-comments.md 作成

NSE binding NOTEs の解消（zzz.R で globalVariables 宣言）

J35S500 examples を で対応（exametrika v1.10.0 CRAN待ち）

vignettes を pkgdown-only articles に移行

examplesの動作確認

ドキュメントの整備（roxygen2）

exametrika v1.10.0 CRAN公開後に を解除

## バージョン1.0.0に向けて

**現在のバージョン: v0.0.35**（2026-02-28時点、mainブランチ）

exametrikaの全プロット機能をggplot2で実装完了したらv1.0.0とする。
exametrika v1.10.0 CRAN公開後、2026年春にリリース予定。
3名体制で並行開発を進める。

### 現在の全体ステータス（2026-02-28更新）

- **バージョン**: v0.0.35（mainブランチ、origin/mainより2コミット先行）
- **Rソースファイル**: 15ファイル（R/ディレクトリ）
- **Export関数**: 31関数（プロット27 + オーバーレイ2 +
  ユーティリティ4、NAMESPACE準拠）
- **共通オプション**:
  全プロット関数で対応済み（一部DAG等はlinetype除外）
- **テスト**: 12ファイル、166テストブロック、489アサーション、FAIL 0 /
  WARN 22 / SKIP 0
- **R CMD check**: 0 errors \| 0 warnings \| 0
  notes（v0.0.35、`--as-cran`オプション付きで完全クリア）
- **CRAN準備**: v0.0.35でCRAN投稿準備完了（cran-comments.md作成済み）
...
- **残タスク**: DAG可視化（LDB, BINET）

### Export関数一覧（31関数、NAMESPACE準拠）

**プロット関数（25種）:** plotICC_gg, plotICRF_gg, plotTRF_gg,
plotIIC_gg, plotTIC_gg, plotIRP_gg, plotFRP_gg, plotTRP_gg, plotLCD_gg,
plotLRD_gg, plotCMP_gg, plotRMP_gg, plotCRV_gg, plotRRV_gg,
plotArray_gg, plotFCRP_gg, plotFCBR_gg, plotScoreField_gg,
plotFieldPIRP_gg, plotLDPSR_gg, plotScoreFreq_gg, plotScoreRank_gg,
plotICRP_gg, plotICBR_gg, plotGraph_gg

**オーバーレイプロット関数（2種）:** plotICC_overlay_gg,
plotIIC_overlay_gg

（合計27種のプロット関数 = 25 + 2オーバーレイ）

**ユーティリティ関数（4種）:** combinePlots_gg, LogisticModel,
ItemInformationFunc, ItemInformationFunc_GRM

### CRAN投稿準備の状態（v0.0.35）

v0.0.35でCRAN投稿に必要な以下の作業を完了した：

- DESCRIPTION: typo修正、Title Case化、Description
  CRAN準拠化（ISBNリファレンス追加）、非標準フィールド除去
- gridExtra: DependsからImportsに移動（CRAN推奨）
- R (\>= 3.5.0): Dependsに追加（最小R版）
- LICENSE: MIT + file LICENSE用のLICENSEファイル作成（2023-2026）
- NSE: zzz.Rに包括的なglobalVariables()宣言（30+変数）。R CMD check
  NOTEゼロ達成
- Examples: J35S500を使うexampleをで保護（exametrika
  v1.10.0がCRAN未公開のため）
- Vignettes: pkgdown-only articlesに移行（R CMD
  checkでのvignetteビルドエラー回避）
- cran-comments.md: 初回投稿用に作成
- README: R-CMD-checkバッジとMITライセンスバッジ追加
- .Rbuildignore: CLAUDE.md大文字小文字不問、cran-comments.md除外

**ブロッカー**: exametrika
v1.10.0のCRAN公開が先行する必要がある（J35S500データセットのを解除するため）

### exametrikaのプロットタイプ一覧と実装状況（exametrika v1.10.0 準拠）

※ 対応モデル欄はexametrikaのplot.exametrika内valid_typesに基づく（注:
一部不正確な宣言あり、下記注釈参照）
...
| **FCRP** | nominalBiclustering, ordinalBiclustering | plotFCRP_gg | **実装済（v1.10.0新規、style対応、共通オプション対応済み）** |
| **FCBR** | ordinalBiclustering | plotFCBR_gg | **実装済（v1.10.0新規、ordinal専用、共通オプション対応済み）** |
| **ScoreField** | nominalBiclustering, ordinalBiclustering | plotScoreField_gg | **実装済（v1.10.0新規、共通オプション対応済み）** |
...
### exametrikaのモデル一覧とプロット対応（plot.exametrika valid_types準拠、v1.10.0）
...
**太字** = v1.10.0で追加された多値バイクラスタリング向けプロット

**注釈:** - (\*1) **FRPのLCA/LRA対応について（2026-02-23調査）**:
exametrikaのplot.exametrika
valid_typesではLCA/LRAに対しFRPが有効と宣言されているが、実際にはLCA()もLRA()も出力に\$FRPフィールドを含まない。exametrikaのplot関数は\`x\$FRP\`に直接アクセスするため、LCA/LRAでFRPを描画しようとするとエラーになる。これはexametrika側のvalid_types宣言の誤りであり、ggExametrikaでは正しくLCA/LRAを拒否している。長男（exametrika）への修正提案事項。
...
#### v1.10.0 多値バイクラスタリングプロット（全完了）

plotFCRP_gg — カテゴリ確率プロット、style パラメータ（line/bar）対応

plotFCBR_gg — 境界確率プロット（ordinal専用）

plotScoreField_gg — 期待得点ヒートマップ（フィールド×クラス/ランク）

#### 多値版対応（全完了）

plotFRP_gg — 2値・多値両対応、stat パラメータ（mean/median/mode）

plotRRV_gg / plotCRV_gg — 2値・多値両対応、stat パラメータ、show_labels

#### その他（全完了）

plotLDPSR_gg — BINET専用（v0.0.32で実装）

#### 共通オプション対応（全27プロット関数 + 2オーバーレイ関数 = 全完了）

全プロット関数で title, colors, linetype, show_legend, legend_position
に対応済み。 （plotGraph_gg のみ linetype 除外：DAGには不適合のため）
...
1.  ~~**BNM（Bayesian Network Model）**~~ —
    項目同士のベイジアンネットワーク → **実装済み・テスト済み** ✅
    - 項目（Item）ノード間の有向グラフ（DAG）を描画する
    - 最も基本的なDAG可視化であり、他の全モデルの土台となる
    - sugiyamaレイアウト、direction=“LR”対応、共通オプション対応済み
2.  **LDLRA（Latent Rank Analysis with Local Dependence）** —
    ランクごとの項目DAG → **基本実装済み（テスト未整備）**
    - BNMの拡張：同じ項目ノード間のDAGが、ランクごとに異なる構造を持つ
    - 実装済み機能:
      全ランク対応、孤立ノード除外、show_prob/prob_digitsパラメータ
    - 残課題: testthatテストの正式整備
    - 参考図: `develop/TDE_figures/LDLRA_TDE02.png`
3.  **LDB（Local Dependence Biclustering）** — ランクごとのフィールドDAG
    → **未実装**
    - LDLRAとの違い：ノードが項目ではなくフィールド（Latent
      Field、緑ダイヤ）
    - ランクごとにフィールドの依存構造が異なる
    - 参考図: `develop/TDE_figures/LDB_TDE01.png`, `LDB_TDE02.png`
4.  **BINET（Bicluster Network Model）** — 統合ネットワーク（最も複雑）
    → **未実装**
    - Latent Class（青四角）とLatent
      Field（緑ダイヤ）の2種類のノードの統合グラフ
    - **あるランクの人間が次のランクに行くために、どのフィールドを経由するかを表現するモデル**
    - 参考図: `develop/TDE_figures/BINET_TDE01.png`, `BINET_TDE02.png`,
      `BINET_CMP_TDE.png`
    - BNM, LDLRA,
      LDBの全てを理解した上で初めて実装できる。絶対に先にやるな

### v1.0.0リリースに向けたロードマップ

**前提**: exametrika v1.10.0
がCRANに公開されている必要がある（ggExametrikaの一部exampleで使用するJ35S500データセットのため）

1.  **LDLRAテスト整備** — testthatテストの正式整備（基本実装済み）
2.  **LDB DAG実装** — ランクごとのフィールドDAG（LDLRA完了後に着手）
3.  **BINET DAG実装** — 統合ネットワーク（LDB完了後に着手）
4.  **exametrika v1.10.0 CRAN公開後**: `\dontrun{}` を `@examplesIf`
    に戻す
5.  **最終R CMD check** → **CRAN投稿（v1.0.0）**

**現時点でCRAN投稿可能な状態**: v0.0.35はR CMD
check完全クリア、cran-comments.md作成済み。DAG可視化が完了前でもCRAN初回投稿は可能（BNM/LDLRAのみ対応状態で投稿し、後のバージョンでLDB/BINETを追加する方針も検討可）。

### v1.1.0以降の予定

- **visNetwork版（plotGraph_interactive）** —
  インタラクティブDAG編集。ノードドラッグ → 座標取得 →
  ggraph清書のワークフロー用
...
**参考画像**: `develop/TDE_figures/` にTest Data
Engineeringからの出力例がある。 これらの図に近づけることが目的。

**plotGraph_gg()** - ggraphで静的ggplotオブジェクト版 -
論文・レポート用の清書版 - カスタム座標を引数で渡すことも可能

**現在の状態 (2026-02-28更新):** - `R/plotGraph_gg.R`
実装済み（434行） - **BNM**: 実装・テスト済み（test-DAG-plots.R:
5テスト、共通オプション対応済み） - **LDLRA**:
plotGraph_gg内にLDLRA処理ブロック追加済み（2026-02-18、log.md参照）。全ランク対応、孤立ノード除外、show_prob/prob_digitsパラメータ対応。ただしtestthat正式テストは未整備 -
**LDB**: 未実装 - **BINET**: 未実装 - 次回: LDLRAのtestthatテスト整備 →
LDB実装 → BINET実装

**v1.1.0以降**:
visNetworkによるインタラクティブ版（plotGraph_interactive）を追加予定。ノードドラッグで位置調整
→ 座標取得 → ggraph清書のワークフローを実現する。
