...
補足:
- gridExtra: v0.0.35でDependsからImportsに移動（CRANではDependsの最小化を推奨）
...
- visNetwork: v1.1.0以降で導入予定（インタラクティブDAG編集用）

### CRANチェックリスト（v0.0.35時点）
- [x] R CMD check --as-cran で 0 errors | 0 warnings | 0 notes（v0.0.35で達成）
- [x] NEWS.mdの更新（v0.0.35まで全バージョン記録済み）
- [x] DESCRIPTIONのバージョン更新（v0.0.35）
...
- [x] J35S500 examples を \dontrun{} で対応（exametrika v1.10.0 CRAN待ち）
- [x] vignettes を pkgdown-only articles に移行
- [x] examplesの動作確認
- [x] ドキュメントの整備（roxygen2）
- [ ] exametrika v1.10.0 CRAN公開後に \dontrun{} を解除
...
- **R CMD check**: 0 errors | 0 warnings | 0 notes（v0.0.35、`--as-cran`オプション付きで完全クリア）
...
- Examples: J35S500を使うexampleを\dontrun{}で保護（exametrika v1.10.0がCRAN未公開のため）
...
**ブロッカー**: exametrika v1.10.0のCRAN公開が先行する必要がある（J35S500データセットの\dontrun{}を解除するため）
...
**前提**: exametrika v1.10.0 がCRANに公開されている必要がある（ggExametrikaの一部exampleで使用するJ35S500データセットのため）

1. **LDLRAテスト整備** — testthatテストの正式整備（基本実装済み）
2. **LDB DAG実装** — ランクごとのフィールドDAG（LDLRA完了後に着手）
3. **BINET DAG実装** — 統合ネットワーク（LDB完了後に着手）
4. **exametrika v1.10.0 CRAN公開後**: `\dontrun{}` を `@examplesIf` に戻す
5. **最終R CMD check** → **CRAN投稿（v1.0.0）**

**現時点でCRAN投稿可能な状態**: v0.0.35はR CMD check完全クリア、cran-comments.md作成済み。DAG可視化が完了前でもCRAN初回投稿は可能（BNM/LDLRAのみ対応状態で投稿し、後のバージョンでLDB/BINETを追加する方針も検討可）。

### v1.1.0以降の予定

- **visNetwork版（plotGraph_interactive）** — インタラクティブDAG編集。ノードドラッグ → 座標取得 → ggraph清書のワークフロー用
...
**v1.1.0以降**: visNetworkによるインタラクティブ版（plotGraph_interactive）を追加予定。ノードドラッグで位置調整 → 座標取得 → ggraph清書のワークフローを実現する。
