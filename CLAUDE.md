...
補足: - ggrepel:
plotCRV_gg/plotRRV_ggのshow_labelsで使用（v0.0.24追加） - tidyr:
plotICBR_gg/plotICRP_ggのpivot_longerで使用（v0.0.18追加） - visNetwork:
インタラクティブDAG編集用として予定（未導入） -
...
4.  ~~**FRP**~~ — 多値版で stat パラメータ（mean/median/mode）対応 →
    ...
    - 共通オプション完全対応（title, colors, linetype, show_legend,
      legend_position）
5.  ~~**RRV/CRV**~~ — 多値版で stat パラメータ（mean/median/mode）対応 →
    実装完了 ✅
    - 2値・多値両対応、stat パラメータ（mean/median/mode）実装済み
...
7.  **LDLRA（Latent Rank Analysis with Local Dependence）** —
    ランクごとの項目DAG → **基本実装済み（テスト未整備）**
    - ランクごとに独立した項目同士のネットワークを描くグラフ
    - BNMの拡張：同じ項目ノード間のDAGが、ランクごとに異なる構造を持つ
    - 参考図:
      `develop/TDE_figures/LDLRA_TDE02.png`（ランクごとに区切られた項目DAG）
    - 実装済み機能:
      全ランク対応、孤立ノード除外、show_prob/prob_digitsパラメータ
    - 残課題:
      testthatテストの正式整備、developスクリプトでの動作確認のみ
8.  **LDB（Local Dependence Biclustering）** — ランクごとのフィールドDAG
    → **未実装**
    ...
    - 参考図:
      `develop/TDE_figures/LDB_TDE01.png`（ランクごとのフィールドDAG横並び）
    - 参考図:
      `develop/TDE_figures/LDB_TDE02.png`（パラメータ推定値重ね表示版）
9.  **BINET（Bicluster Network Model）** — 統合ネットワーク（最も複雑）
    → **未実装**
    - 項目全体を通じて、ランク（Latent
      Class）のつながりとフィールド（Latent
      Field）のつながりをネットワークで表す統合グラフ
    - **あるランクの人間が次のランクに行くために、どのフィールドを経由するかを表現するモデル**
    - Latent Class（青四角）とLatent
      Field（緑ダイヤ）の2種類のノードを同時にプロットしなければならない
    - クラス間のエッジ上にフィールドが配置される構造
    - 参考図:
      `develop/TDE_figures/BINET_TDE01.png`（クラス+フィールドの統合ネットワーク）
    - 参考図: `develop/TDE_figures/BINET_TDE02.png`（Local Dependence
      PSRs重ね表示版）
    - 参考図:
      `develop/TDE_figures/BINET_CMP_TDE.png`（グラフ上にCMP/LCDを表示した版）
    - BNM, LDLRA,
...
plotICC_gg — 共通オプション対応済み (v0.0.27)

plotIIC_gg — 共通オプション対応済み (IRT/GRM両対応)

plotTIC_gg — 共通オプション対応済み
(IRT/GRM両対応、v0.0.27でcolor→colors改名)

plotTRF_gg — 共通オプション対応済み (v0.0.27)

plotIRP_gg — 共通オプション対応済み (v0.0.27)

plotFRP_gg — 共通オプション対応済み（2値・多値両対応、stat
パラメータ含む）

plotTRP_gg — 共通オプション対応済み (v0.0.27、colors\[1\]=bar fill,
colors\[2\]=line color)

plotLCD_gg — 共通オプション対応済み (v0.0.27、colors\[1\]=bar fill,
colors\[2\]=line color)

plotLRD_gg — 共通オプション対応済み (v0.0.27、colors\[1\]=bar fill,
colors\[2\]=line color)

plotCMP_gg — 共通オプション対応済み (v0.0.27)

plotRMP_gg — 共通オプション対応済み (v0.0.27)

plotArray_gg — 共通オプション対応済み（多値データ対応含む）

plotFieldPIRP_gg — 共通オプション対応済み (v0.0.27)

plotGraph_gg — 共通オプション対応済み
(v0.0.27、title/colors/show_legend/legend_position。linetypeはDAGには不適合のため除外)

plotScoreFreq_gg — 共通オプション対応済み (v0.0.15)

plotScoreRank_gg — 共通オプション対応済み (v0.0.19)

plotICRP_gg — 共通オプション対応済み (v0.0.18)

plotICBR_gg — 共通オプション対応済み (v0.0.18)

plotICC_overlay_gg — 共通オプション対応済み (v0.0.16)

plotIIC_overlay_gg — 共通オプション対応済み (v0.0.17)

#### 新規実装予定関数（v1.9.0対応、共通オプションは実装時に検討）

plotFCRP_gg — v1.9.0新規（style パラメータ: line/bar）実装済み

plotFCBR_gg — v1.9.0新規（ordinal専用）実装済み

plotScoreField_gg — v1.9.0新規（ヒートマップ）実装済み
...
**現在の状態 (2026-02-25更新):** - `R/plotGraph_gg.R`
