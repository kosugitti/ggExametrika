...
K個のカテゴリを持つ項目に対して、P(応答 >= k)（k = 1, ..., K-1）を表示します。
...
順序尺度（多値）のバイクラスタリングでは、`plotFCBR_gg()` でフィールド累積境界参照を
可視化できます。各フィールドの境界確率 P(Q >= k) を潜在クラス/ランクごとに表示します。
...
利用可能なレイアウト: `"sugiyama"`（階層型）、`"fr"`（Fruchterman-Reingold）、
