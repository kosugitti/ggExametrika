---
title: "ggExametrika 入門ガイド"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ggExametrika 入門ガイド}
...
**ggExametrika** は、[exametrika](https://kosugitti.github.io/Exametrika/)
...
| 関数 | 説明 |
|-----|------|
| `plotICC_gg()` | 項目特性曲線 (Item Characteristic Curve) |
| `plotIIC_gg()` | 項目情報曲線 (Item Information Curve) |
| `plotTIC_gg()` | テスト情報曲線 (Test Information Curve) |
| `plotTRF_gg()` | テスト応答関数 (Test Response Function) |

### GRM (段階反応モデル)

| 関数 | 説明 |
|-----|------|
| `plotICRF_gg()` | 項目カテゴリ応答関数 (Item Category Response Function) |
| `plotIIC_gg()` | 項目情報曲線（GRM版） |
| `plotTIC_gg()` | テスト情報曲線（GRM版） |

### 潜在クラス・ランクモデル

| 関数 | 説明 |
|-----|------|
| `plotIRP_gg()` | 項目参照プロファイル (Item Reference Profile) |
| `plotFRP_gg()` | フィールド参照プロファイル (Field Reference Profile) |
| `plotTRP_gg()` | テスト参照プロファイル (Test Reference Profile) |
| `plotLCD_gg()` | 潜在クラス分布 (Latent Class Distribution) |
| `plotLRD_gg()` | 潜在ランク分布 (Latent Rank Distribution) |
| `plotCMP_gg()` | クラスメンバーシッププロファイル (Class Membership Profile) |
| `plotRMP_gg()` | ランクメンバーシッププロファイル (Rank Membership Profile) |

### バイクラスタリング

| 関数 | 説明 |
|-----|------|
| `plotCRV_gg()` | クラス参照ベクトル (Class Reference Vector) |
| `plotRRV_gg()` | ランク参照ベクトル (Rank Reference Vector) |
| `plotArray_gg()` | 配列プロット (Array Plot) |
| `plotFieldPIRP_gg()` | フィールドPIRPプロット (Field PIRP Plot) |

### DAG可視化

| 関数 | 説明 |
|-----|------|
| `plotGraph_gg()` | 有向非巡回グラフ (Directed Acyclic Graph) |

### ユーティリティ

| 関数 | 説明 |
|-----|------|
| `combinePlots_gg()` | 複数プロットのグリッド配置 |
| `LogisticModel()` | 4パラメータロジスティックモデル |
| `ItemInformationFunc()` | IRT項目情報関数 |
| `ItemInformationFunc_GRM()` | GRM項目情報関数 |
